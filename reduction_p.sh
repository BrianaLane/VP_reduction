...
if [ ! -x $CUREVP/subtractfits ]; then
    echo "Unable to find $FT/subtractfits, please check that CUREVP is"
    echo "  set correctly."
    exit 1
fi
...
if [ ! -x $CUREVP/deformer3 ]; then
	echo "Unable to find $FT/deformer3, please check that CUREVP is"
	echo "  set correctly."
	exit 1
fi
...
# Run command $1 in parallel with $3 as arguments and write output to $2, failed commands are logged to $ERRORLOG
    tolog $@
    echo $3 | tr ' ' '\n' | $CUREVP/parallel "$1 >> $2 2>&1 || echo \"Command: \'$1\' in `pwd` failed\" >> $ERRORLOG "
}
...
# Run command $1 in parallel with $2 as arguments and write output to $3, failed commands are logged to $ERRORLOG
    tolog $@
    echo $4 | tr ' ' '\n' | $CUREVP/parallel -X "$1 || echo \"Command: \'$2\' in `pwd` failed\" >> $ERRORLOG " >> $3 2>&1
}
...
function deform
{
    ...
    echo "Running deform for science frames"
